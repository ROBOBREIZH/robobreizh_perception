...
def get_limb_position(candidate, subset, id):
    index = int(subset[0][id])
    if index == -1:
        return [None, None]
...
def get_key_limbs_position(candidate, subset):
    arr = []
...
def draw(img, arr):
    for dic in arr:
        for name in dic:
            x, y = dic[name]
            if x != 0 and y != 0:
                cv2.circle(img, (int(x), int(y)), 6, [255, 0, 255], thickness=-1)
...
def draw_bodypose(canvas, candidate, subset):
    ...
    names = ["Neck", "ElbowR", "ElbowL", "HipR", "HipL"]
    ids = [1, 2, 5, 8, 11]
    for name, id in zip(names, ids):
        print(name + ": ", get_limb_position(candidate, subset, id))
